...
export function Page({ children, title, loading, action, actionSlot, hasPermission = true }: IPage) {
  if (!hasPermission) {
    return (
      <div className="flex w-full justify-center">
        <Alert variant="destructive">
          ...
          <AlertDescription>Você não possui permissão para entrar nessa página</AlertDescription>
        </Alert>
      </div>
    );
  }
    ...
    <>
      {(title) && (
        <div className="flex flex-col mb-4">
          <div className="flex items-center justify-between">
            ...
            {actionSlot ? actionSlot : action ? <RenderActions action={action} /> : null}
          </div>
        </div>
      )}
      ...
      {loading ? (
        <div className="flex w-full justify-center pt-8">
          <LoaderCircle data-testid="loader-page" className="animate-spin text-muted-foreground" />
        </div>
      ) : (
        children
      )}
        ...
        if ("to" in singleAction) {
          return (
            <Link
              ...
              className={cn(buttonVariants({ variant: "outline" }), "max-h-[28px] py-1 px-2")}
              to={singleAction.to}
            >
              {content}
            </Link>
